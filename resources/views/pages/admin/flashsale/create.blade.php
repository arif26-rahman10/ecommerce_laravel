...
<script>
    ...
    // Add event listeners for 'focus' on start_time and end_time fields
    document.getElementById('start_time').addEventListener('focus', function () {
        if (!this.value) {
            this.value = getFormattedDateTime(); // Set current time if field is empty
        }
    });

    document.getElementById('end_time').addEventListener('focus', function () {
        if (!this.value) {
            this.value = getFormattedDateTime(1); // Set time +1 hour if field is empty
        }
    });
</script>
@endsection